...
- New **`@cookieyes/scripts`** package with ready-made presets (Segment and Meta Pixel; Google to follow) plus a `customScript` helper for any other tag. Pass them to the `integrations` config: `initCookieYes({ integrations: [segment({ writeKey })] })`.
